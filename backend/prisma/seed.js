...
async function main() {
    ...
    await Promise.all(permissions.map((permission) => prisma.permission.upsert({
        where: { name: permission.name },
        update: permission,
        create: permission,
    })));
    await Promise.all(widgets.map((widget) => prisma.dashboardWidget.upsert({
    ...
    const rolePermissionData = Object.entries(rolePermissionAssignments).flatMap(([roleName, permissionNames]) => {
