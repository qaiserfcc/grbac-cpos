...
const roleWidgetAssignments: Record<string, string[]> = {
...
async function main() {
  ...
  const roleByName: Record<string, (typeof roleRecords)[number]> = {};
  roleRecords.forEach((role: (typeof roleRecords)[number]) => {
    roleByName[role.name] = role;
  });

  const permissionByName: Record<string, (typeof permissionRecords)[number]> = {};
  permissionRecords.forEach((permission: (typeof permissionRecords)[number]) => {
    permissionByName[permission.name] = permission;
  });

  const widgetByKey: Record<string, (typeof widgetRecords)[number]> = {};
  widgetRecords.forEach((widget: (typeof widgetRecords)[number]) => {
