...
export function Modal({ title, description, isOpen, onClose, children, footer, widthClass = "max-w-lg" }: ModalProps) {
  if (!isOpen) return null;

  const modalContent = (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      <div className="absolute inset-0 bg-slate-900/80 backdrop-blur-sm" aria-hidden="true" onClick={onClose} />
      <div className={clsx("glass relative z-10 w-full rounded-2xl border border-white/20 p-6 shadow-2xl backdrop-blur-md", widthClass)}>
        <button
          type="button"
          onClick={onClose}
