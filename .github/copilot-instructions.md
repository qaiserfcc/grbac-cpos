...
Waiting for Review
==****==
Take 1 Task at a time and change status 'In Progress' from the list of available tasks in autoplans.ds and do the following: 
1. Check if the memory-bank/ directory exists in the project.
2. If it exists, read all memory bank files in the following order:
   a. productContext.md
   b. activeContext.md
   c. systemPatterns.md
   d. decisionLog.md
   e. progress.md
 2A. Check existing code regarding task already in progress - continue from current state,if already completed - update status, or blocked tasks and handle accordingly.
3. Set the Memory Bank status to '[MEMORY BANK: ACTIVE]'.
4. Use the context from the Memory Bank to complete the task.
  - complete test and verify the task completeion by thoroughly checking the codebase, running tests, and ensuring no errors and mark task complete.
  - Commit code changes to github after each playwright task comes in Queue and both playwright and jest tests pass successfully.
  - Update the relevant memory bank files if any significant changes occur in the project during the task.
  - If any significant changes occur in the project during the task, update the relevant memory bank files accordingly.
5. Continue to the next task in autoplans.ds and repeat the process until all tasks are completed.
